...
for segment_name, segment_data in segments_new.items():
    ...
    scatter = sns.scatterplot(
        ...
        x='CompactionIndex',
        y='AverageSize',
        hue='NumberOfNiches',  # Add Number of Niches as hue
        size=segment_data['Predicted_PlaneSizeRange'] == segment_data['True_PlaneSizeRange'],  # Size based on correct/wrong classification
        sizes={True: 300, False: 100},  # Larger size for correct matches, smaller for mismatches
        style='True_PlaneSizeRange',
        markers=markers,
        palette='viridis',  # Use a gradient color palette for Number of Niches
    )
    sns.regplot(data=segment_data, x='CompactionIndex', y='AverageSize', color='black', scatter=False)
    plt.colorbar(scatter.collections[0], label='Number of Niches')  # Add a colorbar
...
# Combine all segments data for the final plots
combined_data = pd.concat(all_segments_data, ignore_index=True)

# Plot real plane area vs predicted plane size range for all segments (with Number of Niches as hue and size)
...
scatter = sns.scatterplot(
    ...
    hue='NumberOfNiches',  # Add Number of Niches as hue
    size=combined_data['Predicted_PlaneSizeRange'] == combined_data['True_PlaneSizeRange'],  # Size based on correct/wrong classification
    sizes={True: 300, False: 100},  # Larger size for correct matches, smaller for mismatches
    style='True_PlaneSizeRange',
    markers=markers,
    palette='viridis',  # Use a gradient color palette for Number of Niches
...
plt.legend(loc='best')  # Place legend inside the plot
plt.savefig(generate_filename(f'{output_dir}/plane_size_vs_predicted_all_segments_niches', 'png'))
plt.close()

# Plot real plane area vs predicted plane size range for all segments (with original style including genotypes)
plt.figure(figsize=(12, 8))
sns.scatterplot(
    data=combined_data,
    x='PlaneSize',
    y='Predicted_PlaneSizeRange',
    hue='Category',  # Original hue based on genotype category
    style='True_PlaneSizeRange',
    markers=markers,
    size=combined_data['Predicted_PlaneSizeRange'] == combined_data['True_PlaneSizeRange'],
    sizes={True: 300, False: 100},  # Larger size for correct matches, smaller for mismatches
    palette=colors,  # Use the original color palette for genotypes
)

# Adding vertical lines to indicate the boundaries between Small, Medium, and Large
plt.axvline(x=1600, color='gray', linestyle='--', label='Small/Medium Boundary')
plt.axvline(x=2400, color='gray', linestyle='--', label='Medium/Large Boundary')
plt.title('Real Plane Area vs Predicted Plane Size Range for All Segments (Genotype Categories)')
plt.xlabel('Plane Size')
plt.ylabel('Predicted Plane Size Range')
plt.legend(loc='best')  # Place legend inside the plot
plt.savefig(generate_filename(f'{output_dir}/plane_size_vs_predicted_all_segments_genotypes', 'png'))
plt.close()
